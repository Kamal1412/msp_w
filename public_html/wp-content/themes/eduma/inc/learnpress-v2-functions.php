...
if ( !function_exists( 'thim_courses_searching_callback' ) ) {
	function thim_courses_searching_callback() {
		...
		$keyword = $_REQUEST['keyword'];
                #  print_r(__LINE__); exit;
		if ( $keyword ) {
			$keyword   = strtoupper( $keyword );
                            
                        global $wpdb;
                        $results = $wpdb->get_results( 
                                        //$wpdb->prepare("SELECT * FROM {$wpdb->prefix}your_table_without_prefix WHERE some_field_in_your_table=%d", $some_parameter) 
                                        $wpdb->prepare("SELECT * FROM  courses WHERE examTitle LIKE %s LIMIT 10", strtolower($keyword) . '%') 
                                        );
                    
        		$newdata = array();
			foreach ( $results as $k => $v) {
				$newdata[] = array(
					'id'    => strtoupper($v->examTitle),
					'title' => strtoupper($v->examTitle) .' - ' .$v->examDesc,
					'guid'  => '',
				);
			}
...
/**
 * Create ajax handle for places searching
 */
if ( !function_exists( 'thim_places_searching_callback' ) ) {
	function thim_places_searching_callback() {
		...
		$keyword = $_REQUEST['keyword'];
 			...
 			$keyword   = strtoupper( $keyword );
                        ...
                        $results = $wpdb->get_results( 
                                        //$wpdb->prepare("SELECT * FROM {$wpdb->prefix}your_table_without_prefix WHERE some_field_in_your_table=%d", $some_parameter) 
                                        $wpdb->prepare("SELECT * FROM  countries_cities WHERE cityname LIKE %s LIMIT 10", $keyword . '%') 
                                        );
			...
			foreach ( $results as $k => $v) {
				$newdata[] = array(
					'id'    => $v->countryid,
					'title' => $v->cityname .', ' .$v->countryname,
					'guid'  => '',
				);
			}
...
print_r($newdata); exit;*/
			...
			if(count($results)){
				...
				$newdata[] = array(
					'id'    => '',
					'title' => '<i>' . esc_html__( 'No place found', 'eduma' ) . '</i>',
					'guid'  => '#',
				);
...
add_action( 'wp_ajax_nopriv_places_searching', 'thim_places_searching_callback' );
add_action( 'wp_ajax_places_searching', 'thim_places_searching_callback' );
...
function thim_control_loading_popup_iframe() {
	...
	<script>
		jQuery(function($){
			$(document).on('click', '.complete-lesson-button, .button-next-question, .button-prev-question, .button-finish-quiz', function(){
				window.parent.jQuery('.thim-loading-container').show();
			});

			setTimeout(function(){
				window.parent.jQuery('.thim-loading-container').hide();
			},2000);
...
//add_action('learn_press_course_item_content', 'thim_control_loading_popup');

function thim_control_loading_popup_parent() {
	?>
	<script>
		jQuery(function($){
			$(document).on('click', '.button-load-item', function(){
				$('.thim-loading-container').show()
			});
		})
	</script>
	<?php
}
//add_action('learn_press_before_single_course', 'thim_control_loading_popup_parent');