...
define( 'THIM_THEME_VERSION', '2.5.3.1' );
...
//Function ajax eduma install demo data
add_action( 'wp_ajax_list_cities', 'list_cities' );
add_action( 'wp_ajax_nopriv_list_cities', 'list_cities' );
/* widget gallery posts ajax output */
...
$sql = "SELECT * FROM `countries_cities` where countryname ='".$_REQUEST['country']."' ORDER BY cityname ASC";  
$results = (array)$wpdb->get_results( $sql, OBJECT );
$newResults = objectToArray($results);

 $cityOptions = '<select id="field_2" name="field_2"> <option value="">----</option>';
 foreach ($newResults as $res) {
  
  $cityOptions.="<option value=$res[cityname]>$res[cityname]</option>";
