...
(function ($) {
	...
	var courses_search_timer = false;

        //Widget live search course
        var places_search_timer = false;
        
	function thimlivesearch(contain, type) {
		var input_search = (type == 'courses') ?  contain.find('.courses-search-input') : contain.find('.places-search-input'),
			list_search = (type == 'courses') ? contain.find('.courses-list-search1') : contain.find('.places-list-search') ,
 		...
 		if (keyword) {
			...
			loading.addClass('fa-spinner fa-spin');
 			$.ajax({
				type   : 'POST',
				data   : 'action='+((type == 'courses') ? 'courses_searching' : 'places_searching')+'&keyword=' + keyword + '&from=search',
				url    : ajaxurl,
				success: function (html) {
					...
					if (!items.error) {
						$.each(items, function (index) {
							if (index == 0) {
								if(this['guid'] != '#') {
									//data_li += '<li class="ui-menu-item' + this['id'] + ' ob-selected" id="' + this['id'] + '"><a class="ui-corner-all" href="' + this['guid'] + '">' + this['title'] + '</a></li>';
									data_li += '<li class="ui-menu-item' + this['id'] + ' ob-selected" id="' + this['id'] + '">' + this['title'] + '</li>';
                                                                }else{
									data_li += '<li class="ui-menu-item' + this['id'] + ' ob-selected">' + this['title'] + '</li>';
								}

							} else {
								data_li += '<li class="ui-menu-item' + this['id'] + '" id="' + this['id'] + '">' + this['title'] + '</li>';
							}
						});
	...
	function thimsearchHover() {
		$('.courses-list-search1 li').on('click', function () {
                    //alert($(this).html())
                    $('input[name="s"]').val($(this).attr('id'));
                    //$('input[name="course"]').val($(this).html())
                    //$('input[name="course"]').val($(this).html())

                        if ( $(".courses-list-search1 li").length > 0) {
				 $(".courses-searching .courses-list-search1").empty();
                                 $('.courses-list-search1').removeClass('search-visible');

				// $(".courses-searching .thim-s").val('');
			}
			//$('.courses-list-search1 li').removeClass('ob-selected');
			//$(this).addClass('ob-selected');
		});
                $('.places-list-search li').on('click', function () {
                     
                    $('input[name="place"]').val($(this).html());
                    //$('input[name="course"]').val($(this).html())
                    //$('input[name="course"]').val($(this).html())
                    if ( $(".places-list-search li").length > 0) {
				 $(".courses-searching .places-list-search").empty();
                                 $('.places-list-search').removeClass('search-visible');
	...
	$(document).ready(function () {
		...
		$(document).on('click', '.search-popup-bg', function () {
			var parent = $(this).parent();
			window.clearTimeout(courses_search_timer);
                        window.clearTimeout(places_search_timer);

			parent.find('.courses-list-search1').empty();
			parent.find('.thim-s').val('');
			$('body').removeClass('thim-search-active');
		});

		$(document).on('keyup', '.courses-search-input', function (event) {
 			clearTimeout($.data(this, 'courses_search_timer'));
			var contain = $(this).parents('.courses-searching'),
				list_search = contain.find('.courses-list-search1'),
			...
			console.log(event.which);
  			if (event.which == 13) {
				...
				var courses_search_timer = setTimeout(function () {
 					thimlivesearch(contain, 'courses');
				}, 500);
				$(this).data('courses_search_timer', courses_search_timer);
			}
		});
                
                $(document).on('keyup', '.places-search-input', function (event) {
 
			clearTimeout($.data(this, 'places_search_timer'));
			var contain = $(this).parents('.courses-searching'),
				list_search = contain.find('.places-list-search'),
				item_search = list_search.find('>li');
			console.log(event.which);
  			if (event.which == 13) {
				console.log('11111111');
				...
				var places_search_timer = setTimeout(function () {
                                      thimlivesearch(contain, 'places');
				}, 500);
				$(this).data('places_search_timer', places_search_timer);
			}
		});
		...
		/*$('.courses-list-search1 li').on('click', function () {
			...
			$(this).addClass('ob-selected');
               ...
               /* $(document).on('keypress click', '.ui-menu-item*', function (event) {
			var item_search = $(this).parents('.courses-searching').find('.courses-list-search1>li');
				...
				if (item_search.length > 1) {
					...
					if (selected.prev().length == 0) {
						selected.siblings().last().addClass("ob-selected");
                                                ...
                                                $('.courses-search-input').val(selected.siblings().last().text());
					} else { // otherwise we just select the next one
						selected.prev().addClass("ob-selected");
                                                $('.courses-search-input').val(selected.prev().text());
                                        }
				}
			}
			if (event.keyCode == 40) {
				...
				if (item_search.length > 1) {
					...
					if (selected.next().length == 0) {
						selected.siblings().first().addClass("ob-selected");
                                                $('.courses-search-input').val(selected.siblings().first().text());

					} else { // otherwise we just select the next one
						selected.next().addClass("ob-selected");
                                                $('.courses-search-input').val(selected.next().text());
		...
		$(document).on('click', '.courses-list-search1, .courses-search-input .places-search-input', function (event) {
		...
		$(window).scroll(function () {
			/*if ($('body').hasClass('course-scroll-remove') && $(".courses-list-search1 li").length > 0) {
				$(".courses-searching .courses-list-search1").empty();
				$(".courses-searching .thim-s").val('');
			}*/
		});
		...
		$(document).on('focus', '#popup-header .thim-s', function(){
			var link = $('#popup-header .courses-list-search1 a');
			console.log( $(this).val() , link.length);
			if( $(this).val() != '' && link.length > 0 ) {
				$('#popup-header .courses-list-search1').addClass('search-visible');
			}
		});

		$(document).on('change', '#filter_am, #filter_pm, #filter_no_pref', function (event) {
                     
			if($(this).attr('id') == 'filter_no_pref' && $(this).is(':checked')){
				$('#filter_am').prop('checked',false);
				$('#filter_pm').prop('checked',false);
			}
			
			if($('#filter_am').is(':checked') || $('#filter_pm').is(':checked')){
				$('#filter_no_pref').prop('checked',false);
			}
	...
	jQuery(window).load(function () {
	jQuery(function ($) {
		$('.login-username [name="log"]').attr('placeholder', "");
		$('.login-password [name="pwd"]').attr('placeholder', thim_js_translate.password);
	});jQuery(function ($) {
			$('#thim_login').attr('placeholder', "Username / Email");
